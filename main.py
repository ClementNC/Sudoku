...
erase = Button(200,540, erase_img, 0.5)
...
pause_btn = Button(10, 530, pause_img, 0.7)
...
class Grid:
    def __init__(self, level, rows, cols, width, height):
        ...
        self.cells = [[Cell(int(self.board[i][j]), int(self.board[i][j]), i, j, width, height) for j in range(cols)] for i in range(rows)]
        self.width = width
        self.height = height
        self.model = None
        self.selected = None
    ...
    def note_check(self, row, col, num):
        for r in range(9):
            remove_notes = False
            if num in self.cells[r][col].note:
        ...
        for c in range(9):
            remove_notes = False
            if num in self.cells[row][c].note:
                if not (is_valid((row,c), num, self.model)):
                    remove_notes = True
            if remove_notes:
        ...
        for i in range(box_y * 3, box_y * 3 + 3):
            for j in range(box_x * 3, box_x * 3 + 3):
                remove_notes = False
                if num in self.cells[i][j].note:
                    if not (is_valid((i, j), num, self.model)):
                        remove_notes = True
                if remove_notes:
    ...
    def place(self, val):
        ...
        if int(self.board[row][col]) == 0:
            ...
            self.update_model()
            self.note_check(row, col, val)

            if is_valid((row,col), int(val), self.model) and solve(self.model):
                self.cells[row][col].wrong = False
                return True
            else:
                self.cells[row][col].note = []
                self.cells[row][col].wrong = True
                self.update_model()
                return False
...
class Cell:
    ...
    def __init__(self, value, is_permanent, row, col, width ,height):
        self.value = value
    ...
    def draw(self, win):
        ...
        if len(self.note) > 0 and self.value == 0:
            ...
            user_text = fnt.render(str(self.value), 1, (143, 150, 245))
            text = fnt.render(str(self.value), 1, (0, 0, 0))
            wrong_text = fnt.render(str(self.value), 1, (255, 0, 0))
            if not self.wrong:
                if self.is_permanent:
                    win.blit(text, (x + (gap/2 - text.get_width()/2), y + (gap/2 - text.get_height()/2)))
                else:
                    win.blit(user_text, (x + (gap/2 - text.get_width()/2), y + (gap/2 - text.get_height()/2)))
            else:
                win.blit(wrong_text, (x + (gap/2 - text.get_width()/2), y + (gap/2 - text.get_height()/2)))
...
def main(level):
    ...
    while run:
        ...
        if erase.get_clicked() and grid.selected != None:
            i,j = grid.selected
            if grid.board[i][j] == '0':
                grid.clear()
        ...
        if pause_btn.get_clicked():
            state = pause(WIN, play_time)
            # Changes the start time or the variable run with False to terminate the loop
            if state == False:
                run = state
            else:
                start = state
